...
execute @s[tag=staffstatus,scores={BBMCC=1}] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§cUAC §¶§b► §6Bottom Bedrock §bhas been toggled §2Caves and Cliffs §bby §d"},{"selector":"@s"}]}
#turn off
execute @s[tag=staffstatus,scores={BBMCC=2}] ~~~ scoreboard players operation @a BBMCC = ftDummy BBMCC
execute @s[tag=staffstatus,scores={BBMCC=2}] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§cUAC §¶§b► §6Bottom Bedrock §bhas been toggled §cy=0 §bby §d"},{"selector":"@s"}]}
