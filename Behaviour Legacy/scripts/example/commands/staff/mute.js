...
Server.command.register(registerInformation, (chatmsg, args) => {
    try {
        ...
        if(playerfound) {
            ...
            try {
                sender.runCommandAsync(`execute as "${playername}" at @s if @s[tag=staffstatus] run tellraw "${name}" {"rawtext":[{"text":"§¶§c§lUAC ► §d${playername} §bis staff and can't be muted"}]}`);
                return;
            } catch {
                TellRB(`flag_0`, `UAC ► ${name} has muted ${playername} from game chat for ${args[0]} minutes`);
                sender.runCommandAsync(`execute as "${playername}" at @s if @s[tag=!staffstatus] run tellraw @a {"rawtext":[{"text":"§¶§c§lUAC ► §d${playername} §bwas muted for §c${args[0]} §bminutes by §d${name}"}]}`);
                sender.runCommandAsync(`execute as "${playername}" at @s run scoreboard players add @s[tag=!staffstatus] chatspam ${muteamount}`);
                sender.runCommandAsync(`execute as "${playername}" at @s run tag @s[tag=!staffstatus] add muted`);
            }
