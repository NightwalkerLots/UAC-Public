        ...
        if (scoreTest(sender, 'icmtoggle') === 0) {
            ...
            if (registerInformation.name.match(chatmsg)) {
                ...
                // tpa accept
                    ...
                    sender.runCommandAsync(`execute as @p[name=!"${name}",scores={tpa=${scoreTest(sender, 'tpa')}}] at @s run tp @s "${name}"`);
                    sender.runCommandAsync(`scoreboard players set @s tp_cooldown 900`); 
                    sender.runCommandAsync(`execute as @p[name=!"${name}",scores={tpa=${scoreTest(sender, 'tpa')}}] at @s run scoreboard players set @s tp_cooldown 900`);
            
                    //effects
                    sender.runCommandAsync(`execute as @p[name=!"${name}",scores={tpa=${scoreTest(sender, 'tpa')}}] at @s run playsound note.pling @s ~ ~ ~`);
                    sender.runCommandAsync(`execute as @p[name=!"${name}",scores={tpa=${scoreTest(sender, 'tpa')}}] at @s run function particle/nether_poof`);
                    sender.runCommandAsync(`execute as @p[name=!"${name}",scores={tpa=${scoreTest(sender, 'tpa')}}] at @s run playsound mob.shulker.teleport @s ~~~ 2 2 2`); 

                    //request reset
                    sender.runCommandAsync(`scoreboard players set @a[scores={tpa=${scoreTest(sender, 'tpa')}}] tpa 0`);
                    sender.runCommandAsync(`execute as @a[tag=tpatemp,scores={tpa=${scoreTest(sender, 'tpa')}}] at @s run tag @s remove tpatemp`);
