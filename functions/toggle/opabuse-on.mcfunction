execute @s[tag=ownerstatus] ~~~ scoreboard players set opamtoggledummy opamtoggle 1
execute @s[tag=ownerstatus] ~~~ scoreboard players operation @a opamtoggle = opamtoggledummy opamtoggle
execute @s[tag=ownerstatus] ~~~ scoreboard players set @a OPAM 1
...
execute @s[tag=ownerstatus] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§cUAC §¶§b► §6Anti OP Abuse §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
