    ...
    typedParams: new cc.typedParams(
        {
            ...
            execute: ([tgl, id], { executer, log }) => {
                ...
                if (m.toggle === tglNew) return log(`Module §e${m.name}§r is currently enabled.`)
            ...
            execute: ([,id, key, value], { executer, log }) => {
                ...
                if (!c) throw new Error(`Module ${m.name} is not configurable`)

                if (key == undefined) return log([
                    ' ',
                ...
                if (!t) throw new Error(`Unknown property '${key}'`)
            ...
            execute: ([], { executer, log }) => {
                ...
                for (const m of module.values()) {
                    if (!m.configParser) continue
                    x.push('§e' + m.name + '§r')
                    for (const [k, v] of Object.entries(m.configParser.values)) x.push(` §8:§r ${k}: ${prettify(m.data[v.key])}`)
                }
                x.push(' ')
                log(x)
            }
