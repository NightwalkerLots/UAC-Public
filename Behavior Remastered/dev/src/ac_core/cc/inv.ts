    ...
    typedParams: new cc.typedParams(
        {
            ...
            execute: async ([,target,id], { executer, log }) => {
                const [plr]: [Player] = target ? await target(executer) : [executer]
            ...
            execute: async ([,target,id], { executer, log }) => {
                ...
                if (!data) throw new Error(`Saved inventory '${sid}' not available`)

                const c = plr.getComponent('inventory').container
                for (let i = 0; i < c.size; i++) c.setItem(i, data.list[i])
            ...
            execute: async ([,target], { executer, log }) => {
                const [plr]: [Player] = await target(executer)

                const c = plr.getComponent('inventory').container
                const cb = executer.getComponent('inventory').container

                for (let i = 0, m = Math.min(cb.size, c.size); i < m; i++) cb.setItem(i, c.getItem(i) ?? undefined)
...
const savedInventory = new Map<string, { list: (ItemStack | undefined)[], slot: number }>()
