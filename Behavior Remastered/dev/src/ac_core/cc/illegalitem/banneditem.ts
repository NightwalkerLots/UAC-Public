...
import illegalItemModule, { bannedItemData } from "../../module/illegalitem.js"; 
    ...
    typedParams: new cc.typedParams(
        {
            ...
            execute: ([, id, { 'banned-data': bannedData, 'creative': requireCreative, 'action': action, 'allowed-data-range': range } = {} as any], {log}) => {
                if (id in illegalItemModule.data.bannedItem) throw new Error(`Item '${id}' is already in banned item list.`)

                const o = illegalItemModule.data.bannedItem[id] = {} as bannedItemData
                if (requireCreative !== undefined) o.requireCreative = requireCreative
                if (action) o.action = action === 'inherit' ? undefined: action
            ...
            execute: ([, id, { 'banned-data': bannedData, 'creative': requireCreative, 'action': action, 'allowed-data-range': range } = {} as any], {log}) => {
                ...
                if (!dat) throw new Error(`Item '${id}' is not banned.`)

                if (requireCreative !== undefined) dat.requireCreative = requireCreative
                if (action) dat.action = action === 'inherit' ? undefined: action
