...
const supportedEnch = new Set<string>([
    'protection',
    'fireProtection',
    'featherFalling',
    'blastProtection',
...
const enchListFlags = new cc.typedFlags(
    new Proxy(
        Object.create(null),
        {
    ...
    typedParams: new cc.typedParams(
        {
            ...
            execute: ([, id, lvl], { log }) => {
                if (!supportedEnch.has(id)) throw new Error(`Enchantment '${id}' is unknown.`)
            ...
            execute: ([,, id, list = {}], { log }) => {
                if (id in illegalItemModule.data.enchOverrides) throw new Error(`Enchantment overrides for '${id}' already exists.`)
            ...
            execute: ([,, id, list], { log }) => {
                ...
                if (!override) throw new Error(`Enchantment overrides for '${id}' not found.`)
                ...
                for (const [k, v] of Object.entries(list)) {
                    ...
                    if (v === 'inherit') delete override[k]
                    else override[k] = v as number
                }
            ...
            execute: ([,, id, ench, lvl], { log }) => {
                ...
                if (!supportedEnch.has(ench)) throw new Error(`Enchantment '${ench}' is unknown.`)
                if (lvl === 'inherit') {
            ...
            execute: ([,, id], { log }) => {
                if (!(id in illegalItemModule.data.enchOverrides)) throw new Error(`Enchantment overrides for '${id}' not found.`)
