...
import server, { HighInterval, HighTimeout } from "../../core/lib/server.js";
...
export class CombatModuleConstructor extends Module<combatModuleData> {
    constructor() {
        super('combat',
            'Combat',
            'Detects KillAura, AutoClicker, Reach.', {
                maxReach: 3,
                maxHorDegTol: 90,
                maxVerDegTol: 45,
                autoAimDegTol: 0.1,
    ...
    events = new EventEmitter<combatEvents>('Module (Combat)')

    combatLogList = new Set<string>()
    combatLogTimer = new WeakMap<Player, HighTimeout>()
    cpsList = new WeakMap<Player, { lastHitTime: number, list: number[] }>()

    detectionImmune = new WeakSet<Player>()
    flagCount = new WeakMap<Player, number>()
    flagCooldownStart = new WeakMap<Player, number>()

    action(plr: Player, detection: string, detail: string) {
            ...
            [C in Sign]:
                And<A, C, 0> extends 1
                    ? B extends 0
                        ? [null, null]
                        : [null, undefined]
                    : [Array2<Array2<0 | 2>>, Array2<Array3<0 | 1 | 2>>]
        }
    }
} = {
    '0': {
...
function clampDeg(v: number, method: 'floor' | 'ceil' = 'ceil') {
    return v - Math[method]((method === 'floor' ? v + 180 : v - 180) / 360) * 360
}

function deg2([x, z]: Array2) {
    return -Math.atan2(x, z) / Math.PI * 180
}

function deg3([x, y, z]: Array3) {
    return -Math.atan2(y, Math.hypot(x, z)) / Math.PI * 180
}

function isInside(min: number, max: number, rot: number) {
    return min <= max ? min < rot && rot <= max : !(max <= rot && rot < min)
}

function getHitDeg(a: Area, b: Array3, hr: number): [Array2, Array2, number[], Sign[]] {
    const ab: Array3<Array3> = [a.from, b, a.to]
    const _a = a.getClosestAxisDistance(b)
    const [i, j, k] = _a.map(Math.sign) as Array3<Sign>

    let [h, v] = kaDirectionMapping[i][j][k]

    let hRange: Array2 = [-Infinity, Infinity], vRange: Array2 = [-Infinity, Infinity]
    ...
    if (v !== null) {
        if (v === undefined) {
            ...
            const planes = ([
                ...
                [ [ 135,-135], [[x1, z1], [x2, z1]] ],
                [ [-135, -45], [[x2, z1], [x2, z2]] ],
            ] as [Array2, Array2<Array2>][]).find( ([[a, b]]) => isInside(a, b, hr) )
            if (!planes) throw Error()
...
world.events.entityHit.subscribe(({ entity: plr, hitEntity: tgt }): any => {
    if (!(
        combatModule.toggle
    ...
    if (combatModule.data.combatLog && permission.getPlayerLevel(tgt) < 80) {
        ...
        combatModule.combatLogTimer.set(tgt, new HighTimeout(() => { try { tgt.name; combatModule.combatLogList.delete(tgt.uidStr) } catch {} }, combatModule.data.combatLogWearoffTime))
    }

    if (permission.getPlayerLevel(plr) >= 80 || combatModule.detectionImmune.has(plr)) return
    ...
    if (!cpsData || ctime - cpsData.lastHitTime > 3000) combatModule.cpsList.set(plr, cpsData = { lastHitTime: ctime - 1000, list: [] })
...
server.addEventListener('tick', ({currentTick, deltaTime}) => {
    if (currentTick % combatModule.data.flagImmuneTime !== 0) return
...
server.addEventListener('playerLoad', plr => {
    if (!( combatModule.toggle && combatModule.combatLogList.has(plr.uidStr) )) return
    combatModule.combatLogList.delete(plr.uidStr)

    if (combatModule.data.combatLog && combatModule.data.combatLogCommand) asyncExecCmd(combatModule.data.combatLogCommand, plr)
})

server.addEventListener('playerLeave', ({playerId: uid, playerName: name}) => {
    const uidStr = playerManager.stringifyUid(uid)
    if (!( combatModule.toggle && combatModule.data.combatLog && combatModule.combatLogList.has(uidStr) )) return
...
export type combatModuleData = {
    ...
    combatLogList: string[]
}

export type combarModuleAction = combatModuleData['automodAction'] | 'alert'

export type combatEvents = {
    action: {
        readonly plr: Player
        readonly action: combarModuleAction
        readonly detection: string
        readonly detail: string
    }
    combatlog: {
        readonly plrName: string
        readonly plrUid: string
    }
}

type Sign = -1 | 0 | 1

type And<T, A, B> = T extends A ? T extends B ? 1 : 0 : 0
type Or<T, A, B> = T extends A ? 1 : T extends B ? 1 : 0
