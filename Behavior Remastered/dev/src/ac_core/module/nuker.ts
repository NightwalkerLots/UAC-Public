...
export class NukerModuleConstructor extends Module<nukerData> {
    constructor() {
    ...
    events = new EventEmitter<nukerEvents>()

    counter = new Map<Player, number>()

    flagCount = new WeakMap<Player, number>()
    flagCooldownStart = new WeakMap<Player, number>()

    action(plr: Player, detection: string, detail: string, force = false) {
...
server.addEventListener('tick', ({ deltaTime }) => {
    ...
    for (const [plr, c] of nukerModule.counter) {
        if (c > Math.ceil(nukerModule.data.maxSpeed * tpsDetectionRate)) nukerModule.action(plr, 'Bad breaking speed', `§7(BPS: §2${c}§7)`)
        if (c > Math.ceil(nukerModule.data.maxHardSpeed * tpsDetectionRate)) nukerModule.action(plr, 'Illegal breaking speed', `§7(BPS: §2${c}§7)`, true)
    }
...
export type nukerEvents = {
    action: {
        readonly plr: Player
        readonly action: automodAction | 'alert'
