...
export class RBRelayModuleConstructor extends Module<RBRelayModuleData> {
    constructor() {
    ...
    readonly prefix = 'RB1337: '

    sendLog(data: RBLogScheme | string) {
    ...
    send(data: any, plr?: Player) {
        if (!plr) [plr] = world.getPlayers({ tags: [this.data.targetTag] })
        if (!plr) return
...
logModule.events.addEventListener('log', data => {
    if (!rbRelayModule.toggle) return
