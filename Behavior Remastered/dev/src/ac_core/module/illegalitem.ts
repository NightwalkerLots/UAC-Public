import { BeforeItemUseEvent, Entity, GameMode, ItemStack, ItemTypes, MinecraftBlockTypes, MinecraftItemTypes, Player, world } from "@minecraft/server"
...
class IllegalItemModuleConstructor extends Module<illegalItemModuleData> {
    constructor() {
        super('illegalitem',
            'Illegal Item',
            'Controls banned item, max enchant levels, shulker scanner',
            {
                testBannedItem: true,
    ...
    events = new EventEmitter<illegalItemEvents>('Module (IllegalItem)')

    sanitizeListCache = new Set<string>()

    // Test as player

    testAsPlayer(plr: Player, item: ItemStack) {
        const itemTest = this.data.testBannedItem && this.testItem(item, creativeList.has(plr))
        if (itemTest) return this.action(plr, itemTest.action ?? this.data.automodAction, 'Banned item', `§7(Item: §2${item.id}§7)`) ? 2 : 1

        const stackTest = this.data.testMaxStack && this.testMaxStack(item)
         if (stackTest) return this.action(plr, this.data.automodAction, 'Illegal stack', `§7(Item: §2${item.id}§7, Stack: §6${stackTest.stack}§7/§2${stackTest.max}§7)`) ? 2 : 1

        const enchTest = this.data.testEnch && this.testEnch(item)
        if (enchTest) return this.action(plr, this.data.automodAction, 'Illegal enchantment', `§7(Item: §2${item.id}§7, Enchant: §2${enchTest.id}§7, Level: §6${enchTest.level}§7/§2${enchTest.enchMaxLvl}§7)`) ? 2 : 1
    ...
    async testAsItemEntity(ent: Entity) {
        ...
        const fail = (detection: string, detail: string) => {
        ...
        if (itemTest) return fail('Banned item', `§7(Item: §2${item.id}§7)`)

        const stackTest = this.data.testMaxStack && this.testMaxStack(item)
         if (stackTest) return fail('Illegal stack', `§7(Item: §2${item.id}§7, Stack: §6${stackTest.stack}§7/§2${stackTest.max}§7)`)
...
// Creative
let creativeList = new WeakSet<Player>()
new server.interval(() => {
    creativeList = new WeakSet
    for (const plr of world.getPlayers({ gameMode: GameMode.creative })) creativeList.add(plr)
}, 1000)

// Checker
server.addEventListener('tick', () => {
    if (!illegalItemModule.toggle) return
...
world.events.blockPlace.subscribe(({player: plr, block}) => {
    ...
    if (
        !illegalItemModule.data.testNonEmptyContainer
    ...
    for (let i = Math.max(0, c.size - 27); i < c.size; i++) {
        for (let i = 0, m = c.size; i < m; i++) {
            ...
            for (let i = Math.max(0, c.size - 27); i < c.size; i++) c.setItem(i, undefined)
...
function testPlrItemUse(ev: BeforeItemUseEvent) {
    const {source: plr, item} = ev
    if (!(illegalItemModule.toggle && plr instanceof Player && permission.getPlayerLevel(plr) < 60)) return
...
const enchSlotCompatibles = (compatibleEnchantments: string[]): Record<string, number> => {
    const obj = Object.create(null)
    for (const ench of compatibleEnchantments) Object.defineProperty(obj, ench, { get() { return illegalItemModule.data.maxEnchLevels[ench] } })
    return obj
};

const enchSlotLevels: Record<number, Record<string, number>> = {
    // None
    0: enchSlotCompatibles([] as []),
    // armorHead
...
export type bannedItemData = {
    action?: bannedItemAction
    requireCreative?: boolean
}

export type bannedItemAction = 'clear' | 'warn' | 'kick' | 'ban' | 'blacklist'

type illegalItemModuleData = {
    testBannedItem: boolean
    bannedItem: Record<string, bannedItemData>

    testMaxStack: boolean
    maxStack: Record<string, number>
    stackSizeDefault: number

    testEnch: boolean
    maxEnchLevels: Record<string, number>
    enchOverrides: Record<string, Record<string, number>>
    ...
    automodAction: bannedItemAction
    automodDuration: number
}

export type illegalItemEvents = {
    action: {
        readonly plr: Player
        readonly action: bannedItemAction
