...
export function getPlayer(name: string) {
    ...
    if (/^(?<!\\)#[0-9a-zA-Z]{3,5}/.test(name)) return playerDat.getFromId(name)

    name = name[0] == '\\' ? name.slice(1) : name
    return playerDat.getFromName(name)
}

export async function kick(plr: string | Player, message: string) {
    try { await asyncExecCmd(`kick ${JSON.stringify(typeof plr === 'string' ? plr : plr.name)} ${message}`) }
    catch {
...
const allowKick = new WeakSet<Player>()

world.events.beforeDataDrivenEntityTriggerEvent.subscribe(ev => {
    const {entity: plr, id} = ev
    if (!(plr instanceof Player && id === 'uac:forcekick')) return
    if (!allowKick.has(plr)) ev.cancel = true
})
