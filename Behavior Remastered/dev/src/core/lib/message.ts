import { CommandResult, Player, world } from "@minecraft/server"
...
export class MessageConstructor {
    ...
    sendMsgToPlayer(target: Player, message: any) {
        const arr = this.convertMsgToArr(message)
    ...
    sendMsgToPlayers(target: Iterable<Player>, message: any, exclude: Player[] = []) {
        const arr = this.convertMsgToArr(message)
        
        for (const plr of target) if (!exclude.includes(plr))
            for (let i = 0; i < arr.length; i += this.linesPerSegment)
                plr.tell( arr.slice(i, i + this.linesPerSegment).join('\n') )
    }

    sendMsgToAdmins(minPermLvl: number, message: any, exclude: Player[] = []) {
        const arr = this.convertMsgToArr(message)
...
for (const obj of [Player, SimulatedPlayer]) {
    Object.defineProperty(obj.prototype, 'sendMsg', {
        configurable: true,
        value: function sendMsg(data: any) {
            if (!(this instanceof obj)) throw new TypeError(`${obj.name}.sendMsg requires 'this' be a '${obj.name}'`)
