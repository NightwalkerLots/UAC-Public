...
export function createFormatter<Args extends any[] = []>(format: string, lccl: LCCL<Args>): (...args: Args) => string {
    let fnList: Record<string, () => any> = new Proxy(Object.create(null), { has: () => true })
    let fnIndex = 0
...
export function setScore(data: ScoreboardIdentity | Entity | string, sb: ScoreboardObjective | string, score: number) {
    ...
    if (sbId) sbId.setScore(sbObj, score)
    else {
        const sbStr = JSON.stringify(sbObj.id)
        if (data instanceof Entity) asyncExecCmd(`scoreboard players set @s ${sbStr} ${score}`, data)
        else asyncExecCmd(`scoreboard players set ${JSON.stringify(data)} ${sbStr} ${score}`)
    }
}
