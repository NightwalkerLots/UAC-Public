...
export class LoaderConstructor {
    ...
    async spawn({x, y, z}: Vector3) {
        while(true) {
            for (const target of (function*() { for (const e of world.getPlayers()) yield e; for (const e of overworld.getEntities({ type: lock.id })) yield e })()) {
