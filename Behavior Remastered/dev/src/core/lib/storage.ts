...
export default class Storage<T = any> extends EventEmitter<storageEvents<T>> {
    static get manifestLock() { return lockManifest }
    static get dataLock() { return lockData }
    declare static cachedList: MapProxy<string, Storage>

    constructor(id: string, name: string, autoload = true, autosaveInterval = 30000) {
    ...
    async write(data: string, chunkSize = 16383) {
    ...
    async read(maxDelay = 10000) {
        ...
        try {
            ...
            const ctrl = new PromiseController<string[] | undefined>()
            ...
            lockManifest.addEventListener('spawn', ent => {
                ...
                manifestLoaded = true
                dataLoadRemaining += ent.getDynamicProperty('size') as number

                lockManifest.kill(ent)
                if (dataLoadRemaining === 0) ctrl.resolve(chunkList)
            }, { abort: ctrl.promise })
            ...
            lockData.addEventListener('spawn', ent => {
                ...
                chunkList[ent.getDynamicProperty('index') as number] = ent.nameTag

                lockData.kill(ent)
                dataLoadRemaining--
                if (manifestLoaded && dataLoadRemaining === 0) ctrl.resolve(chunkList)
            }, { abort: ctrl.promise })

            if (!await structure.asyncLoad(this.id, loc)) ctrl.resolve(undefined)
            new server.timeout(delay => ctrl.reject(new RangeError(`Timeout reached (${delay}ms)`)), maxDelay)

            if (await ctrl.promise === undefined) return
    ...
    async 'delete'() {
        if (this.emit('delete', undefined).cancel) return false

        return await structure.asyncDelete(this.id)
    }
}

const storageList = new Map<string, Storage>()
...
const loaded = Promise.all([ centerPos, server.initialize ]).then(async ([ { center: [x, z] } ]) => {
    if (world.getDynamicProperty('uacx:sload')) return
    await loader.spawn(Vec3([x, 1023, z]))

    while(true) {
        try {
            for (let i = -64; i < 320; i++) overworld.getBlock(Vec3([x, i, z])).setType(MinecraftBlockTypes.air)
            break
        } catch {}
...
const ioActiveList = new Set<string>()
const freeSlotList = new Set<number>( Array.from(Array(384), (v,i) => i - 64) )

async function lock(id: string) {
    if (ioActiveList.has(id)) throw new TypeError(`Storage '${id}' is currently busy`)
    ioActiveList.add(id)

    let slot
    while (( [slot] = freeSlotList, typeof slot !== 'number' )) await server.nextTick
...
async function unlock(id: string, slot: number) {
    if (!ioActiveList.has(id)) return
...
async function clean(slot: number) {
