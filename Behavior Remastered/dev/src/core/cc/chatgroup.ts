    ...
    typedParams: new cc.typedParams(
        {
            ...
            execute: ([,id, priority, tagFilter], { log }) => {
                if (chat.group.has(id)) throw new Error(`Chat group '${id}' already exists`)
            ...
            execute: ([,id,, val], { log }) => {
                ...
                if (!cg) throw new Error(`Chat group '${id}' not found`)
            ...
            execute: ([,id], { log }) => {
                if (!chat.group.has(id)) throw new Error(`Chat group '${id}' not found`)
