...
import cc, { CC } from "../lib/cc.js";
import { clamp, testTagFilter } from "../lib/misc.js";
import permission from "../lib/permission.js";

function canView(plr: Player, cc: CC) {
    ...
    typedParams: new cc.typedParams(
        {
            ...
            execute: ([trigger], { executer, log } ) => {
                ...
                if (!cmd || !canView(executer, cmd)) throw new Error(`Command not found: '${trigger}'`)
                ...
                log(cmd.usage ? [
                    ...
                    ...cmd.usage.map(({usage, description}) => {
                        const usageStr = usage.map(v =>
                            typeof v == 'string' ? `§a${v}§r`
                            : Array.isArray(v) ? '( ' + v.map(v => `§a${v}§r`).join(' | ') + ' )'
                            : `{§b${v.name}§r${v.optional ? '?' : ''}: ${cc.formatType(v)}}`
                        ).join(' ')
