...
cc.create('adminchat', {
    ...
    typedParams: new cc.typedParams(
        {
            ...
            execute: ([], { executer, log }) => {
                if (playerList.has(executer)) return log(`You are currently in the admin chat.`)
            ...
            sequence: ['leave'],
            execute: ([], { executer, log }) => {
                if (!playerList.has(executer)) return log(`You are currently not in the admin chat.`)
            ...
            execute: async ([,selector], { executer, log }) => {
                const [target] = await selector() as [Player]
                if (playerList.has(target)) return log(`§d${target.name}§r is currently in the admin chat.`)
            ...
            execute: async ([,selector], { executer, log }) => {
                const [target] = await selector() as [Player]
                if (!playerList.has(target)) return log(`§d${target.name}§r is currently not in the admin chat.`)
    ...
    onDelete: () => {
        if (adminChatGroup.isListed) chat.group.delete('adminchat')
...
const fnChat = chat.addEventListener('chat', (evd, ctrl): void => {
    const {sender, group} = evd
    if (group?.id !== adminChatGroup.id) return
    if (!playerList.has(sender)) return ctrl.cancel() // illegal

    evd.targets = evd.targets.filter(plr => playerList.has(plr))
})

const fnRole = role.addEventListener('messageFormat', (evd) => {
    if (!playerList.has(evd.player)) return

    evd.format = '§eAdmin Chat§r > ' + evd.format
})

const fnJoin = server.addEventListener('playerLoad', plr => {
    if (plr.removeTag(adminChatTag)) plr.tell('Disconnected from admin chat.')
})
