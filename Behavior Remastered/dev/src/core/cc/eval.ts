...
export const evalContext: any = {
    ...
    inv: (plr: mc.Player) => { return plr.getComponent('inventory').container },
    cls: (plr: mc.Player) => { for (let i = 0; i < 100; i++) plr.tell('\n') },

    [Symbol.toStringTag]: 'Self',
    [Symbol.unscopables]: {},
}

const evalInternalContext: any = {
    mc,
    gt,
    mcui,

    $_: undefined,
    $p: (n: string) => { return mc.world.getAllPlayers().find(v => v.name === n) },
    $t: (n: string) => { return Array.from(overworld.getEntities({ type: n.includes(':') ? n : 'minecraft:' + n })) },
    $o: (n: string) => { return mc.world.scoreboard.getObjective(n) },
}

const vctx: Record<string, any> = new Proxy(evalContext, {
    has: () => true,
    get: (t, p) => {
        //@ts-expect-error
        if (p in globalThis) return globalThis[p]
        if (p in evalContext) return evalContext[p]
        if (p in evalInternalContext) return evalInternalContext[p]

        //@ts-expect-error
        if (p in mc) return mc[p]
        //@ts-expect-error
        if (p in gt) return gt[p]
        //@ts-expect-error
        if (p in mcui) return mcui[p]

        //@ts-expect-error
        if (p in form) return form[p]
        //@ts-expect-error
        if (p in message) return typeof message[p] == 'function' ? message[p].bind(message) : message[p]
    ...
    deleteProperty: (t, p) => {
        //@ts-expect-error
        if (p in globalThis) delete globalThis[p]
        return true
    }
})
...
cc.create('eval', {
    ...
    execute: ({ executer, argFull, log }) => {
        if (argFull) execEval(executer, argFull)
        else {
...
function execEval (plr: mc.Player, cmd: string) {
    ...
    try {
        ...
        if (v instanceof Promise) v.then(
            v => log( '(In promise) ' + misc.prettify(v)),
            e => log( '(in promise) Uncaught ' + ( e instanceof Error ? `${e}\n${e.stack}` : misc.prettify(e) ) )
        )
...
server.addEventListener('beforeChat', (evd, ctrl): void => {
    ...
    else execEval(evd.sender, evd.message)
}, { priority: 1 })
