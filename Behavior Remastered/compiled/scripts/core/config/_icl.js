...
//// CHAT GROUP ////
for (const { id, priority, tag_filter } of chatGroups)
    chat.group.create(id, priority, tag_filter);
//// PERMISSION ////
for (const [k, v] of Object.entries(permissionKeys))
    try {
        ...
        console.error(`[UAC ICL] ERR! Permission: ${e instanceof Error ? e.message : e}`);
    }
permission.opLevel = permissionOperatorLevel;
//// ROLE ////
for (const { id, position, default_role, roles: rolelist } of roles)
    role.group.create(id, position, default_role, rolelist);
