...
export default class EntityLock extends EventEmitter {
    constructor(id, killId = 'uacx:kill') {
        if (server.initialized)
            throw new TypeError(`EntityLock instance must be created before world is initialized`);
        super();
        this.id = id;
        this.killId = killId;
        server.addEventListener('initialize', ({ propertyRegistry }) => {
            ...
            world.events.entitySpawn.subscribe(({ entity }) => {
                if (entity.id !== this.id)
                    return;
                if (!(this.#spawnOpen || entity.getDynamicProperty('lock')))
                    return void this.kill(entity);
            ...
            for (const ent of overworld.getEntities({ type: this.id })) {
                if (this.#spawnables.has(ent))
                    this.#spawnables.delete(ent);
                else if (!ent.getDynamicProperty('lock'))
                    this.kill(ent);
            }
        ...
        world.events.beforeDataDrivenEntityTriggerEvent.subscribe(evd => {
            const { entity, id } = evd;
            if (id == killId && entity.id === this.id && !this.#killables.has(entity))
                return void (evd.cancel = true);
            this.emit('kill', entity);
            return;
        });
    }
    #killables = new WeakSet();
    #spawnables = new WeakSet();
    #spawnOpen = false;
    id;
    killId;
    spawn(dim = overworld, spawnLoc) {
        this.#spawnOpen = true;
        try {
