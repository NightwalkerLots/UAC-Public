import { DynamicPropertiesDefinition, Entity, EntityTypes, Player, PropertyRegistry, World, world } from "@minecraft/server";
import { SimulatedPlayer } from "@minecraft/server-gametest";
import { getStackTrace } from "../lib/misc.js";
export class dpropConstructor {
    get entity() { return entRegList; }
    getEntity(entity) { return entRegList[typeof entity === 'string' ? entity : entity.id]; }
...
const definitionList = new WeakMap();
...
DynamicPropertiesDefinition.prototype.defineString = function (id, ml) {
    if (!(this instanceof DynamicPropertiesDefinition))
        throw new TypeError;
...
//// REGISTRY ////
const entRegList = Object.create(null);
const worldReg = {
    definitions: Object.create(null),
    size: 0
};
...
PropertyRegistry.prototype.registerEntityTypeDynamicProperties = function (def, type) {
    if (!(this instanceof PropertyRegistry))
        throw new TypeError;
...
function getDynamicPropertiesOf(target) {
    const obj = Object.create(null);
    const type = EntityTypes.get(target.id), regData = entRegList[type.id];
    if (!regData)
        return obj;
    for (const k in regData.definitions)
        Object.defineProperty(obj, k, { get() { return target.getDynamicProperty(k); } });
    return obj;
}
