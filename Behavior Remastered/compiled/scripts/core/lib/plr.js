...
export class PlayerConstructor extends EventEmitter {
    get uidShift() { return (world.getDynamicProperty('ushift') ?? 0) + 2 ** 32; }
    parseUid(uid) {
        if (/^[~@#*=\/]/.test(uid))
            uid = uid.slice(1);
        const num = parseInt(uid, 36) - this.uidShift;
        if (isNaN(num))
            throw new SyntaxError(`Invalid UID`);
        return num;
    }
    stringifyUid(num) {
    ...
    createPlayerMap() {
        const destroy = new WeakMap();
        const out = new MapProxy({
            set: (m, plr, v) => {
                m.set(plr, v);
                if (destroy.has(plr))
                    return;
            ...
            delete: (m, plr) => {
                ...
                if (d)
                    playerManager.for(plr).removeEventListener('destroy', d);
                destroy.delete(plr);
                return m.delete(plr);
            },
            clear: (m) => {
...
server.addEventListener('initialize', ({ propertyRegistry }) => {
    ...
    if (world.getDynamicProperty('ushift') == undefined)
        world.setDynamicProperty('ushift', randomIntBetween(36 ** 3, 36 ** 4 - 36 ** 3));
}, { priority: 100 });
...
const playerProperties = new WeakMap();
const playerObjects = new WeakMap();
system.events.scriptEventReceive.subscribe(({ id, message, sourceEntity: plr }) => {
    if (!(plr instanceof Player))
        return;
    if (!playerObjects.has(plr))
        playerObjects.set(plr, new PlayerManager(plr)); // ensure it exists
    const o = playerObjects.get(plr), p = playerProperties.get(plr);
    if (!o || !p)
        return; // what
    const v = message === '1';
    const nid = id.substring(6);
    if (v) {
        p.add(nid);
    }
    else {
    ...
    if (msapiEventList.has(nid))
        o.emit(nid, v); // emit event (non-state)
    else if (nid.startsWith('is_') && msapiEventStateList.has(nid.substring(3)))
        o.emit(v ? nid.substring(3) : nid.substring(3) + '_end', v); // emit event (state)
}, { namespaces: ['msapi'] });
//// PROPERTY ////
...
Object.defineProperties(Player.prototype, {
    nameTag: {
        'get'() {
            if (!(this instanceof Player))
                throw new TypeError(`Getter of 'Player.prototype.nameTag' requires 'this' be a 'Player'`);
            return plrNtGet?.call(this);
        },
        'set'(v) {
            if (!(this instanceof Player))
                throw new TypeError(`Setter of 'Player.prototype.nameTag' requires 'this' be a 'Player'`);
            const evd = {
                plr: this,
                newName: v
            };
            ...
            if (!cancel)
                plrNtSet?.call(this, v);
        }
    },
    uidStr: {
        'get'() {
            if (!(this instanceof Player))
                throw new TypeError(`Getter of 'Player.prototype.uidStr' requires 'this' be a 'Player'`);
    ...
    uid: {
        'get'() {
            if (!(this instanceof Player))
                throw new TypeError(`Getter of 'Player.prototype.uid' requires 'this' be a 'Player'`);
            //@ts-expect-error
    ...
    uidRaw: {
        'get'() {
            if (!(this instanceof Player))
                throw new TypeError(`Getter of 'Player.prototype.uidRaw' requires 'this' be a 'Player'`);
            //@ts-expect-error
