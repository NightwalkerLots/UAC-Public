    ...
    typedParams: new cc.typedParams({
        ...
        execute: ([, key, level], { log }) => {
            if (permission.has(key))
                throw new Error(`Permission key "${key}" is already registered`);
            permission.set(key, level);
            if (level >= 100)
                log(`§eWARNING! Setting permission level of key to 100 or above is not recommended.`);
            log([
        ...
        execute: ([, key1, key2], { log }) => {
            if (!permission.has(key1))
                throw new Error(`Permission key "${key1}" is not registered`);
            if (permission.has(key2))
                throw new Error(`Permission key "${key2}" is already registered`);
        ...
        execute: ([, key], { log }) => {
            if (!permission.has(key))
                throw new Error(`Permission key "${key}" is not registered`);
        ...
        execute: async ([, , selector], { executer, log }) => {
            log([
                ...
                ...Array.from(await selector(executer, { noSelector: false }), plr => ` §8:§r §d${plr.name}§r: ${permission.getPlayerKey(plr) ? `§7"§r${permission.getPlayerKey(plr)}§7"§r -> §a${permission.getPlayerLevel(plr)}§r` : `§8<unset>§r`}`),
                ` `,
            ]);
        }
    }, {
        ...
        execute: async ([, , selector, key], { executer, log }) => {
            const [plr] = await selector(executer);
