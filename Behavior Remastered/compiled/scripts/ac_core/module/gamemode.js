...
export class GamemodeModuleConstructor extends Module {
    constructor() {
    ...
    events = new EventEmitter('Module (Gamemode)');
    listCache = new Map();
}
...
server.addEventListener('tick', () => {
    ...
    for (const gm of gmList) {
        for (const plr of world.getPlayers({ gameMode: gm })) {
            ...
            if (!data || data.allowedGamemodes.has(gm))
                continue;
