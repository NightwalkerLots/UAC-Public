...
class ChatModuleConstructor extends Module {
    constructor() {
        super('chat', 'Chat <Passive>', 'Controls (Server / Shadow) Mute / Unmute, Slowmode, Max chat length', {
            slowmode: 3000,
            maxLength: 80,
            mutes: [],
            serverMute: {
    ...
    events = new EventEmitter('Module (Chat)');
    mute(plr, mod = '[System]', reason = 'No reason', duration = Infinity, muteType = 'normal') {
        if (plr instanceof Player && permission.getPlayerLevel(plr) >= 60)
            throw new Error('This player cannot be muted.');
    ...
    unmute(plr, mod = '[System]', reason = 'No reason') {
        const data = this.muteCache.get(plr.uidStr);
        if (!data)
            throw new Error('This player is not muted.');
    ...
    serverMute(mod = '[System]', reason = 'No reason', duration = Infinity, muteType = 'normal') {
        const modData = mod instanceof Player ? { name: mod.name, uid: mod.uidStr } : { name: mod, uid: '' };
        const expire = Math.min(Date.now() + duration, Number.MAX_VALUE);
    ...
    serverUnmute(mod = '[System]', reason = 'No reason') {
        if (this.data.serverMute.expired)
            throw new Error('Server is not muted.');
...
uacStorage.addEventListener('load', () => {
    ...
    for (const d of chatModule.data.mutes)
        chatModule.muteCache.set(d.uid, d);
})();
chat.addEventListener('chat', (ev, ctrl) => {
    ...
    { // 1nd test: mute
        ...
        if (!serverMute.expired) {
            ...
            if (delta <= 0)
                chatModule.serverUnmute(undefined, 'Mute has expired');
            else
                return fail(`§eServer is currently muted by ${serverMute.muterName} and will expire in ${convertToTime(delta)}. \nMute reason: ${serverMute.reason}`);
        }
        const playerMute = chatModule.muteCache.get(plr.uidStr);
        if (playerMute) {
            const delta = playerMute.expires - ctime;
            if (delta <= 0)
                chatModule.unmute(plr, undefined, 'Mute has expired');
            else {
                if (playerMute.type == 'shadow')
                    ev.targets = [plr];
                else
                    return fail(`§cYou are currently muted by ${playerMute.muterName} and will expire in ${convertToTime(delta)}. \nMute reason: ${playerMute.reason}`);
            }
        }
    }
    { // 2nd test: message length
        const lenMax = chatModule.data.maxLength;
        if (message.length > lenMax)
            return fail(`§cNessage is too long! Maximum length is ${lenMax} characters.`);
    }
    { // 3st test: cooldown
        const delta = (chatModule.cooldown.get(plr) ?? 0) - ctime;
        if (delta > 0)
            return fail(`§cToo fast! Please wait ${(delta / 1000).toFixed(2)} seconds before sending another message.`);
    }
...
new server.interval(() => {
    ...
    const serverMute = chatModule.data.serverMute;
    if (!serverMute.expired && serverMute.expires <= ctime)
        chatModule.serverUnmute(undefined, 'Mute has expired');
}, 3000);
new server.interval(() => {
    const ctime = Date.now();
...
chatModule.events.addEventListener('mute', ({ muter, target, duration, reason, type }) => {
    ...
    if (plrMod)
        plrMod.tell(`${type == 'hard' ? 'Hard-muted' : type == 'shadow' ? 'Shadow-muted' : 'Muted'} §d${targetName}§r §7(§2#${target.uidStr}§7)§r for §a${convertToTime(duration)}§r. \nReason: §a${reason}`);
    if (plrTarget && type !== 'shadow')
        plrTarget.tell(`§eYou have been ${type == 'hard' ? 'hard-muted' : 'muted'} by ${modName}§r§e for ${convertToTime(duration)}. \nReason: ${reason}`);
    if (plrTarget && type === 'hard')
        asyncExecCmd('ability @s mute true', plrTarget);
});
server.addEventListener('playerLoad', plr => {
    ...
    if (!mute)
        return;
    if (mute.type !== 'shadow')
        plr.tell(`§eYou are currently muted by ${mute.muterName}§r§e and will expire in ${convertToTime(mute.expires - Date.now())}. \nMute reason: ${mute.reason}`);
    if (mute.type === 'hard')
        asyncExecCmd('ability @s mute true', plr);
});
chatModule.events.addEventListener('unmute', ({ unmuter, target, reason, oldType }) => {
    ...
    if (plrMod)
        plrMod.tell(`Unmuted §d${targetName}§r §7(§2#${target.uidStr}§7)§r. \nReason: §a${reason}`);
    if (plrTarget && oldType !== 'shadow')
        plrTarget.tell(`§aYou have been unmuted by §d${modName}§r§a. \nReason: §b${reason}`);
    if (plrTarget && oldType === 'hard')
        asyncExecCmd('ability @s mute false', plrTarget);
});
chatModule.events.addEventListener('serverMute', ({ muter, duration, reason, type }) => {
    const plrMod = muter instanceof Player ? muter : undefined;
    const modName = plrMod?.name ?? muter;
    if (plrMod)
        plrMod.tell(`${type == 'hard' ? 'Hard-muted' : 'Muted'} server for §a${convertToTime(duration)}§r. \nReason: §a${reason}`);
    for (const plr of world.getPlayers()) {
        if (plr == muter)
            continue;
        if (permission.getPlayerLevel(plr) >= 60)
            plr.tell(`§3[§bUAC§3]§r §d${modName}§r ${type == 'hard' ? 'hard-muted' : 'muted'} server for §a${convertToTime(duration)}§r. \nReason: §a${reason}`);
        else
            plr.tell(`§eServer has been ${type == 'hard' ? 'hard-muted' : 'muted'} by ${modName}§r§e for ${convertToTime(duration)}. \nReason: ${reason}`);
        if (type === 'hard')
            asyncExecCmd('ability @s mute true', plr);
    }
});
...
chatModule.events.addEventListener('serverUnmute', ({ unmuter, reason, oldType }) => {
    ...
    const modName = plrMod?.name ?? unmuter;
    if (plrMod)
        plrMod.tell(`Unmuted server. \nReason: §a${reason}`);
    for (const plr of world.getPlayers()) {
        if (plr == unmuter)
            continue;
        if (permission.getPlayerLevel(plr) >= 60)
            plr.tell(`§3[§bUAC§3]§r §d${modName}§r unmuted server. \nReason: §a${reason}`);
        else
            plr.tell(`§aServer has been unmuted by §d${modName}§r§a. \nReason: §b${reason}`);
        if (oldType === 'hard')
            asyncExecCmd('ability @s mute false', plr);
    }
});
