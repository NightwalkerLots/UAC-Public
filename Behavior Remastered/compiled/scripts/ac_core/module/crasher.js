...
export class CrasherModuleConstructor extends Module {
    constructor() {
    ...
    events = new EventEmitter('Module (Crasher)');
}
...
server.addEventListener('tick', () => {
    if (!crasherModule.toggle)
        return;
    for (const plr of world.getPlayers()) {
        const pLoc = convertLocationType('Array', plr.location);
        if (permission.getPlayerLevel(plr) >= 80 || boundArea.isInside(pLoc))
            continue;
