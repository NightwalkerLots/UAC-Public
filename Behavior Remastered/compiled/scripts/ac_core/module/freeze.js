...
export class FreezeModuleConstructor extends Module {
    events = new EventEmitter('Module (freeze)');
    list = new Map();
    add(mod, target) {
        if (this.list.has(target.uid))
            throw new Error(`Player ${target.name} (#${target.uidStr}) is already frozen`);
    ...
    remove(mod, target) {
        ...
        if (!this.list.delete(uid))
            throw new Error(`Player ${name} (#${uidStr}) is not frozen.`);
...
uacStorage.addEventListener('load', ({ data }) => {
    ...
    for (const [k, v] of data.freeze)
        freezeModule.list.set(k, v);
});
...
freezeModule.events.addEventListener('remove', ({ mod, target }) => {
    ...
    if (target instanceof Player)
        target.sendMsg(`§aYou are no longer frozen!§r`);
