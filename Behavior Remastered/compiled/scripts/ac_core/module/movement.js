...
export class MovementModuleConstructor extends Module {
    constructor() {
    ...
    events = new EventEmitter('Module (Movement)');
    correctionList = new WeakMap();
    prevVelocity = new WeakMap();
    flagCount = new WeakMap();
    flagCooldownStart = new WeakMap();
    action(plr, detection, detail, correction) {
        ...
        if (correction && flagCount >= this.data.correctionThreshold)
            plr.teleport(correction, plr.dimension, plr.rotation.x, plr.rotation.y);
...
server.addEventListener('tick', ({ deltaTime }) => {
    ...
    for (const plr of world.getPlayers()) {
        ...
        if (!list)
            movementModule.correctionList.set(plr, list = []);
        list.push(plr.location);
        if (list.length > movementModule.data.correctionTicks)
            list.shift();
        ...
        if (accel > movementModule.data.maxAcceleration * tpsRate)
            movementModule.action(plr, 'Bad acceleration', `§7(Accel: §2${accel.toFixed(2)} b/t§7)`, correction);
        if (spd > movementModule.data.maxVelocity * tpsRate)
            movementModule.action(plr, 'Bad speed', `§7(Speed: §2${spd.toFixed(2)} b/t§7)`, correction);
    }
});
