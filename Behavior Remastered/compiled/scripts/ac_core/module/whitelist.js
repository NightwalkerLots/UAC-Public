...
export class WhitelistModuleConstructor extends Module {
    nameList = new Set();
    uidList = new Map();
    events = new EventEmitter('Module (Whitelist)');
    add(plr, mod = '[System]') {
    ...
    remove(plr, mod = '[System]') {
        const plrNew = typeof plr == 'string' ? getPlayer(plr) ?? plr : plr;
        if (typeof plrNew !== 'string') {
            if (!this.uidList.has(plrNew.uidStr))
                throw new Error(`This player is not whitelisted: '${plrNew.name}' (#${plrNew.uidStr})`);
            ...
            if (!this.nameList.has(plrNew))
                throw new Error(`This player is not whitelisted: '${plrNew}'`);
...
uacStorage.addEventListener('load', ({ data }) => {
    ...
    for (const v of data.whitelist.name)
        whitelistModule.nameList.add(v);
    for (const [uid, v] of data.whitelist.uid)
        whitelistModule.uidList.set(uid, v);
});
// Handler
whitelistModule.addEventListener('enable', () => {
    for (const plr of world.getPlayers()) {
        if (permission.getPlayerLevel(plr) >= 60 || whitelistModule.uidList.has(plr.uidStr))
            continue;
        kick(plr, `Whitelist has been enabled. You are not whitelisted.`);
    }
});
server.addEventListener('playerLoad', (plr, ctrl) => {
    if (!whitelistModule.toggle || permission.getPlayerLevel(plr) >= 60)
        return;
    ...
    if (whitelistModule.uidList.has(plr.uidStr))
        return;
    ctrl.break();
...
whitelistModule.events.addEventListener('add', ({ mod, value }) => {
    ...
    if (plrMod)
        plrMod.tell(`Whitelisted §d${targetName}§r §7(${targetUid ? `§2#${targetUid}§7` : `<uid unknown>`})§r.`);
});
whitelistModule.events.addEventListener('remove', ({ mod, value }) => {
    ...
    if (plrMod)
        plrMod.tell(`Removed §d${targetName}§r §7(${targetUid ? `§2#${targetUid}§7` : `<uid unknown>`})§r from whitelist.`);
    if (plrTarget && whitelistModule.toggle && permission.getPlayerLevel(plrTarget) < 60)
        kick(plrTarget, `You are no longer whitelisted in this server.`);
});
