...
export class BlacklistModuleConstructor extends Module {
    list = new Map();
    events = new EventEmitter('Module (Blacklist)');
    add(plr, mod = '[System]', reason = 'No reason') {
        if (plr instanceof Player && permission.getPlayerLevel(plr) >= 80)
            throw new Error('This player cannot be blacklisted.');
    ...
    remove(plr, mod = '[System]', reason = 'No reason') {
        const data = this.list.get(plr.uidStr);
        if (!data)
            throw new Error('This player is not blacklisted.');
...
uacStorage.addEventListener('load', ({ data }) => {
    ...
    for (const v of data.blacklist)
        blacklistModule.list.set(v.uid, v);
});
// Handler
...
server.addEventListener('playerLoad', (plr, ctrl) => {
    if (!blacklistModule.toggle)
        return;
    const d = blacklistModule.list.get(plr.uidStr);
    if (!d)
        return;
    if (permission.getPlayerLevel(plr) >= 80)
        return blacklistModule.remove(plr, undefined, 'Admin cannot be blacklisted');
    ctrl.break();
    kick(plr, [
...
blacklistModule.events.addEventListener('add', ({ mod, target, reason }) => {
    ...
    if (plrMod)
        plrMod.tell(`Blacklisted §d${targetName}§r §7(§2#${target.uidStr}§7)§r. \nReason: §a${reason}`);
    if (plrTarget && blacklistModule.toggle)
        kick(plrTarget, [
            `You have been §4blacklisted§r.`,
...
blacklistModule.events.addEventListener('remove', ({ mod, target, reason }) => {
    ...
    if (plrMod)
        plrMod.tell(`Removed §d${targetName}§r §7(§2#${target.uidStr}§7)§r from blacklist. \nReason: §a${reason}`);
});
