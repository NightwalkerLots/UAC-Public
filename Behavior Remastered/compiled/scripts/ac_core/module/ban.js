...
export class banModuleConstructor extends Module {
    ...
    ban(plr, mod = '[System]', reason = 'No reason', duration = Infinity) {
        if (plr instanceof Player && permission.getPlayerLevel(plr) >= 80)
            throw new Error('This player cannot be banned.');
    ...
    unban(plr, mod = '[System]', reason = 'No reason') {
        const data = this.list.get(plr.uidStr);
        if (!data)
            throw new Error('This player is not banned.');
...
uacStorage.addEventListener('load', ({ data }) => {
    ...
    for (const v of data.ban)
        banModule.list.set(v.uid, v);
});
new server.interval(() => {
...
server.addEventListener('playerLoad', (plr, ctrl) => {
    ...
    if (td <= 0 || permission.getPlayerLevel(plr) >= 80)
        return banModule.unban(plr, undefined, 'Ban has expired');
    ctrl.break();
    kick(plr, [
...
banModule.events.addEventListener('ban', ({ mod, target, duration, reason }) => {
    ...
    if (plrMod)
        plrMod.tell(`Banned §d${targetName}§r §7(§2#${target.uidStr}§7)§r for §a${convertToTime(duration)}§r. \nReason: §a${reason}`);
    if (plrTarget)
        kick(plrTarget, [
...
banModule.events.addEventListener('unban', ({ mod, target, reason }) => {
    ...
    if (plrMod)
        plrMod.tell(`Unbanned §d${targetName}§r §7(§2#${target.uidStr}§7)§r. \nReason: §a${reason}`);
});
