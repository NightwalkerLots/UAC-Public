...
cc.create('uac:log', {
    ...
    typedParams: new cc.typedParams({
        ...
        execute: async ([, { from = new Date(0), to = new Date(864e13), page = 1, listperpage = 72, fast = false } = {}, fi = {}], { executer, log }) => {
            log('Please wait.');
            ...
            let i = 0, l = [];
            l: for await (const data of fast ? logModule.readAll() : logModule.readAllSlow()) {
                i++;
                if (data.timestamp > etime)
                    continue;
                if (data.timestamp < stime)
                    break;
                //@ts-expect-error
                for (const [k, v] of fient)
                    if (data[k] !== v)
                        continue l;
                if (c++ < sl)
                    continue;
            ...
            if (l.length === 0)
                ...
                log([
                    ...
                    ...l.map(([i, data]) => ` §8:§r §7#${i.toString().padStart(4, '0')} ${new Date(data.timestamp).toLocaleString()}§r ` + (data.type === 'detection' ? `§e${data.module}§r/§c${data.detection}§r -> §d${data.name}§r (§2#${data.uid}§r) (§e${data.action}§r) ${data.detail}§r`
        ...
        execute: async ([, i], { executer, log }) => {
            ...
            if (!entry)
                throw new Error('No data in this index.');
            log([
                ' ',
