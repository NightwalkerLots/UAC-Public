...
cc.create('uac:gm', {
    ...
    typedParams: new cc.typedParams({
        ...
        execute: ([, id, priority, filter, gamemode], { log }) => {
            if (gamemodeModule.listCache.has(id))
                throw new Error(`Gamemode group ID '${id}' already exists`);
            if (gamemode.length < 1)
                throw new Error('Allowed gamemodes must be atleast 1');
        ...
        execute: ([, id, filter], { log }) => {
            ...
            if (!d)
                throw new Error(`Gamemode group ID '${id}' not found`);
            d.filter = filter;
        ...
        execute: ([, id], { log }) => {
            if (!gamemodeModule.listCache.delete(id))
                throw new Error(`Gamemode group ID '${id}' not found`);
        ...
        execute: ([], { log }) => {
            log([
                ...
                ...Array.from(gamemodeModule.listCache, ([k, v]) => [
                    ` §8:§r ${k}:`,
