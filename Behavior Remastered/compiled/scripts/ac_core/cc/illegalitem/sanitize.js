...
cc.create('uac:sanitize', {
    ...
    typedParams: new cc.typedParams({
        ...
        execute: ([, id], { log }) => {
            if (illegalItemModule.sanitizeListCache.has(id))
                throw new Error(`Item '${id}' is already in sanitize list`);
        ...
        execute: ([, id], { log }) => {
            if (!illegalItemModule.sanitizeListCache.delete(id))
                throw new Error(`Item '${id}' not listed in sanitize list`);
        }
    })
});
